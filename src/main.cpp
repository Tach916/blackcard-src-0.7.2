...
const string strMessageMagic = "BlackCard Signed Message:\n";
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal BlackCard miner
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in BlackCard
    bool fStrictPayToScriptHash = true; // Always active in BlackCard
    ...
    // ppcoin: fees are not collected by miners as in BlackCard
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "BlackCard", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "All this has happened before, and all this will happen again.";
        CTransaction txNew;
        ...
        txNew.strTxComment = "text:BlackCard genesis block";
        ...
        assert(block.hashMerkleRoot == uint256("0x86a857dfa82d157dffe295a2b7b0915a05a3ca436af9cd38ad4d43f8538effe0"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xe9, 0xe9, 0xe9, 0xe9 };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    if (hash > hashTarget && pblock->IsProofOfWork())
        return error("BlackCardMiner : proof-of-work not meeting target");

    //// debug print
    printf("BlackCardMiner:\n");
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BlackCardMiner : generated block is stale");
        ...
        if (!ProcessBlock(NULL, pblock))
            return error("BlackCardMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ThreadBlackCardMiner(void* parg);

static bool fGenerateBlackCard = false;
static bool fLimitProcessors = false;
static int nLimitProcessors = -1;

void BlackCardMiner(CWallet *pwallet, bool fProofOfStake)
{
    printf("CPUMiner started for proof-of-%s\n", fProofOfStake? "stake" : "work");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);

    // Make this thread recognisable as the mining thread
    RenameThread("BlackCard-miner");

    // Each thread has its own key and counter
    CReserveKey reservekey(pwallet);
    unsigned int nExtraNonce = 0;

    while (fGenerateBlackCard || fProofOfStake)
    {
        if (fShutdown)
            return;
        while (vNodes.empty() || IsInitialBlockDownload())
        {
            ...
            if (fShutdown)
                return;
            if ((!fGenerateBlackCard) && !fProofOfStake)
                return;
        }
        ...
        printf("Running BlackCardMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBlackCardMiner(void* parg)
{
    ...
    try
    {
        ...
        BlackCardMiner(pwallet, false);
        vnThreadsRunning[THREAD_MINER]--;
    }
    catch (std::exception& e) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(&e, "ThreadBlackCardMiner()");
    } catch (...) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(NULL, "ThreadBlackCardMiner()");
    }
    nHPSTimerStart = 0;
    if (vnThreadsRunning[THREAD_MINER] == 0)
        dHashesPerSec = 0;
    printf("ThreadBlackCardMiner exiting, %d threads remaining\n", vnThreadsRunning[THREAD_MINER]);
}


void GenerateBlackCard(bool fGenerate, CWallet* pwallet)
{
    fGenerateBlackCard = fGenerate;
    nLimitProcessors = GetArg("-genproclimit", -1);
    if (nLimitProcessors == 0)
        fGenerateBlackCard = false;
    ...
    if (fGenerate)
    {
        ...
        printf("Starting %d BlackCardMiner threads\n", nAddThreads);
        for (int i = 0; i < nAddThreads; i++)
        {
            if (!NewThread(ThreadBlackCardMiner, pwallet))
                printf("Error: NewThread(ThreadBlackCardMiner) failed\n");
            Sleep(10);
        }
    }
}
