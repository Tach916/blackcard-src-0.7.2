...
static bool ipcScanCmd(int argc, char *argv[], bool fRelay)
{
    ...
    {
        if (boost::algorithm::istarts_with(argv[i], "BlackCard:"))
        {
...
static void ipcThread(void* pArg)
{
    ...
    RenameThread("BlackCard-gui-ipc");
...
void ipcInit(int argc, char *argv[])
{
    ...
    try {
        ...
        // Make sure we don't lose any BlackCard: URIs
        for (int i = 0; i < 2; i++)
        {
        ...
        // Make sure only one BlackCard instance is listening
        message_queue::remove(BOUNTYCOINURI_QUEUE_NAME);
        delete mq;
